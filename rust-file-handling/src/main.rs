...
fn main() {
    ...
    let data = read_by_vector("testFile.txt").unwrap();
    println!("{:?}", data);

    println!("Printing the content using read_to_string");

    println!("{}", read_by_string("testFile.txt").unwrap());

    println!("Printing the content using Bufreader");

    println!("{}", read_by_bufreader("testFile.txt").unwrap());

    println!("Writting the content to a file");
    write_by_clearing(".\\samba.txt", "samba file sa sa sa samba file");
    println!("Writting the content to a file");
    write_by_appendings(".\\samba.txt", "sidddd samba file sa sa sa samba file");
}
